...
export function ResourcePalette({ onAddResource }: ResourcePaletteProps) {
    ...
    return (
        <div className="flex-1 flex flex-col h-full bg-white dark:bg-[#20333a] border-r border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 select-none overflow-hidden font-['Outfit']">
            {/* Header */}
            <div className="px-4 py-3 flex items-center justify-between bg-slate-50/50 dark:bg-[#20333a] backdrop-blur-sm border-b border-slate-200 dark:border-slate-800">
                <span className="text-base font-bold text-slate-800 dark:text-slate-100">Resource Palette</span>
            </div>
                ...
                {Object.entries(filteredCategories).sort().map(([cat, list]) => (
                    <div key={cat} className="group/cat text-left">
                        <button
                            ...
                            className="flex items-center gap-2 w-full px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 border-b border-slate-100 dark:border-slate-800/30"
                        >
                                ...
                                {list.map((snippet) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className="flex items-center gap-3 w-full px-8 py-2.5 hover:bg-slate-100 dark:hover:bg-indigo-500/10 text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-300 transition-all text-sm group/item text-left relative overflow-hidden"
                                        >
